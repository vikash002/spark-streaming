...
sealed trait EmbeddedKafkaSupport {
  ...
  /**
    ...
    * @param messages the [[String]] message to publish
    * @param config an implicit [[EmbeddedKafkaConfig]]
    * @throws KafkaUnavailableException if unable to connect to Kafka
    */
  def publishStringMessageToKafka(topic: String, messages: String*)(implicit config: EmbeddedKafkaConfig): Unit =
    publishToKafka(topic, messages:_*)(config, new StringSerializer)

  /**
    * Publishes synchronously a message to the running Kafka broker.
    *
    * @param topic the topic to which publish the message (it will be auto-created)
    * @param messages the message of type [[T]] to publish
    * @param config an implicit [[EmbeddedKafkaConfig]]
    * @param serializer an implicit [[Serializer]] for the type [[T]]
    * @throws KafkaUnavailableException if unable to connect to Kafka
    */
  @throws(classOf[KafkaUnavailableException])
  def publishToKafka[T](topic: String, messages: T*)
    ...
    val sendResults: ListBuffer[Try[RecordMetadata]] = ListBuffer()
    for (message <- messages) {
      val sendFuture = kafkaProducer.send(new ProducerRecord(topic, message))
      sendResults.append(Try {
        sendFuture.get(5, SECONDS)
      })
    }
    kafkaProducer.close()

    sendResults.find(_.isFailure) match {
      case Some(sendResult) =>
  ...
  def consumeFirstMessageFrom[T](topic: String)(implicit config: EmbeddedKafkaConfig, decoder: Decoder[T]): T = {
    ...
    try {
      val messageWaitingTime = config.messageWaitingTimeInSecs
      Await.result(messageFuture, messageWaitingTime seconds)
    } finally {
      consumer.shutdown()
    }
  }
